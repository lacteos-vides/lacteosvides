...
import { updateGalleryItem, updateGalleryItemWithUrl, type ActionResult } from "../../actions";
import {
  validateImageSize,
  formatFileSize,
  MAX_SIZE_MB,
} from "@/lib/image-upload";
import { uploadImageWithProgress } from "@/lib/image-upload";
...
export function EditGalleryForm({ item }: Props) {
  ...
  const [state, formAction] = useActionState(
    (prev: ActionResult, fd: FormData) => updateGalleryItem(item.id, prev, fd),
  ...
  useEffect(() => {
    if (state.ok && !lastSuccessRef.current) {
      ...
      toast.success("Imagen actualizada", "Los cambios se guardaron correctamente.");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await updateGalleryItemWithUrl(item.id, { ok: false, errors: {} }, saveData);
  ...
  return (
    ...
    >
      <div>
        ...
        />
        {!state.ok && "errors" in state && state.errors?.product && (
      ...
      <div>
        ...
        <input
          ...
          className="w-full rounded-lg border border-slate-300 px-4 py-2.5 text-slate-900 placeholder-slate-400 focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/20 disabled:opacity-70"
        />
        {!state.ok && "errors" in state && state.errors?.price && (
      ...
      <div>
        <p className="mb-2 text-sm font-medium text-slate-700">Imagen actual</p>
      ...
      <div>
        <label className="mb-2 flex cursor-pointer items-center gap-2">
          ...
          />
          <span className="text-sm font-medium text-slate-700">Reemplazar con nueva imagen</span>
        </label>
        {replaceFile && (
          <div className="mt-3 flex flex-col gap-3">
            ...
            {(fileError || (!state.ok && "errors" in state && state.errors?.image)) && (
              <p className="text-sm text-red-600">
                {fileError || ("errors" in state ? state.errors?.image : "")}
              </p>
            )}
          </div>
        )}
